...
func TestTurnIdempotencyClaimCommitAndReplay(t *testing.T) {
	...
	result, err := db.ClaimTurnIdempotency("idem-story", "key-1", "owner-a", time.Minute)
	if err != nil {
	...
	replay, err := db.ClaimTurnIdempotency("idem-story", "key-1", "owner-b", time.Minute)
	if err != nil {
	...
	eventsJSON, found, err := db.GetTurnIdempotency("idem-story", "key-1")
	if err != nil {
...
func TestTurnIdempotencyClaimBlocksAndReclaimsExpiredOwner(t *testing.T) {
	...
	first, err := db.ClaimTurnIdempotency("idem-reclaim", "key-1", "owner-a", 20*time.Millisecond)
	if err != nil {
	...
	if _, err := db.ClaimTurnIdempotency("idem-reclaim", "key-1", "owner-b", time.Minute); !errors.Is(err, ErrTurnIdempotencyInProgress) {
		t.Fatalf("second claim err = %v, want ErrTurnIdempotencyInProgress", err)
	}

	time.Sleep(35 * time.Millisecond)
	second, err := db.ClaimTurnIdempotency("idem-reclaim", "key-1", "owner-b", time.Minute)
	if err != nil {
