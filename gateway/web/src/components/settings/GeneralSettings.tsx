...
export function GeneralSettings({ preferences, onChange }: { preferences: AppPreferences; onChange: (preferences: AppPreferences) => void }) {
  ...
  const applyAccent = (accent: string) => {
    const accentHistory = [accent, preferences.accent, ...preferences.accentHistory]
  ...
  return (
    <div className="general-settings-stack">
      <section className="settings-group" aria-labelledby="general-interface-title">
        <header><div><h4 id="general-interface-title">{t("settings_ui:general.interfaceTitle")}</h4><p>{t("settings_ui:general.interfaceDesc")}</p></div></header>
        <div className="settings-field-grid">
          <div className="settings-field settings-span-full" data-setting-id="interface-language">
      ...
      <section className="settings-group" aria-labelledby="general-color-title" data-setting-id="accent">
      ...
      <section className="settings-group" aria-labelledby="general-layout-title">
        ...
        <div className="settings-toggle-list">
          <SettingsToggle id="stories-sidebar" label={t("options:storiesSidebar")} description={t("settings_ui:general.storiesSidebarDesc")} checked={preferences.showLeftRail} onChange={(checked) => update("showLeftRail", checked)} />
          <SettingsToggle id="inspector" label={t("options:inspector")} description={t("settings_ui:general.inspectorDesc")} checked={preferences.showInspector} onChange={(checked) => update("showInspector", checked)} />
          <SettingsToggle id="transcript-wrap" label={t("options:wrap")} description={t("settings_ui:general.wrapDesc")} checked={preferences.wrapTranscript} onChange={(checked) => update("wrapTranscript", checked)} />
        </div>
      </section>
    </div>
  );
...
export function SettingsToggle({ id, label, description, checked, onChange }: { id: string; label: string; description: string; checked: boolean; onChange: (checked: boolean) => void }) {
  return (
    <label className="settings-toggle" data-setting-id={id}>
      ...
      <span className="switch-control"><input type="checkbox" checked={checked} onChange={(event) => onChange(event.target.checked)} /><span aria-hidden="true" /></span>
    </label>
  );
}
