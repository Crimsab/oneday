...
func (db *DB) CreateSession(s *Session) error {
	_, err := db.conn.Exec(
		`INSERT INTO sessions (id, story_id, started_at, ended_at, summary, branch_id, source_commit_id)
		 VALUES (?, ?, ?, ?, ?,
		   COALESCE(NULLIF(?,''),(SELECT active_branch_id FROM stories WHERE id=?)),
		   COALESCE(NULLIF(?,''),(SELECT b.head_commit_id FROM story_branches b JOIN stories st ON st.active_branch_id=b.id WHERE st.id=?)))`,
		s.ID, s.StoryID, s.StartedAt, s.EndedAt, s.Summary,
		s.BranchID, s.StoryID, s.SourceCommitID, s.StoryID,
	)
	if err != nil {
...
func (db *DB) GetSession(id string) (*Session, error) {
	...
	).Scan(&s.ID, &s.StoryID, &s.StartedAt, &s.EndedAt, &s.Summary, &s.BranchID, &s.SourceCommitID)
	if err != nil {
...
func (db *DB) ListSessions(storyID string) ([]Session, error) {
	...
	for rows.Next() {
		var s Session
		if err := rows.Scan(&s.ID, &s.StoryID, &s.StartedAt, &s.EndedAt, &s.Summary, &s.BranchID, &s.SourceCommitID); err != nil {
