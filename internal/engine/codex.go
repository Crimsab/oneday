...
func BuildStoryCodex(db *storage.DB, story *storage.Story, char *storage.Character, world *storage.WorldState) (*CodexIndex, error) {
	...
	for _, npc := range npcs {
		...
		projection, _ := db.GetPlayerSafeEntity(story.ID, npc.CanonicalEntityID, observerID, turn)
		addEntry(buildNPCCodexEntry(&npc, projection))
	}
	...
	canonicalFactions, _ := db.ListPlayerSafeFactions(story.ID)
	if len(canonicalFactions) > 0 {
		for _, faction := range canonicalFactions {
			lines := []string{}
			for _, member := range faction.KnownMembers {
				lines = append(lines, fmt.Sprintf("%s — %s (%s)", member.EntityID, member.Role, member.Status))
			}
			if char != nil {
				if score, ok := faction.Reputation[char.ID]; ok {
					lines = append(lines, fmt.Sprintf("Reputation: %d", score))
				}
			}
			entry := CodexEntry{ID: "factions:canonical:" + faction.ID, Category: "factions", Title: faction.Name, Summary: faction.ProfileJSON}
			if len(lines) > 0 {
				entry.Sections = []CodexSection{{Title: "Known Relations", Lines: lines}}
			}
			addEntry(entry)
		}
	} else {
...
func buildNPCCodexEntry(npc *storage.NPC, projection *storage.PlayerSafeEntityProjection) CodexEntry {
	...
	entry := CodexEntry{
		...
		CanonicalEntityID: npc.CanonicalEntityID,
		Category:          "people",
		Title:             npc.Name,
	...
	if projection != nil {
		if strings.TrimSpace(projection.DisplayName) != "" {
			entry.Title = projection.DisplayName
		}
		lines := []string{}
		for _, fact := range projection.Facts {
			lines = append(lines, fmt.Sprintf("%s: %s", strings.ReplaceAll(fact.Predicate, "_", " "), fact.ObjectJSON))
		}
		if len(lines) > 0 {
			entry.Sections = append(entry.Sections, CodexSection{Title: "Known Facts", Lines: lines})
		}
	}
