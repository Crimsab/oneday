...
func (db *DB) CreateNPC(npc *NPC) error {
	return db.WithTx(func(tx *sql.Tx) error { return db.CreateNPCTx(tx, npc) })
}
...
func (db *DB) CreateNPCTx(tx *sql.Tx, npc *NPC) error {
	...
	return db.syncNPCCompatibilityTx(tx, npc)
}

const createNPCSQL = `
		INSERT INTO npcs (id, story_id, canonical_entity_id, name, role, appearance, personality_json, private_thoughts,
         relationship_json, nemesis_json, discovery_json, notes_on_protagonist, desires, disposition, is_alive, first_appeared_turn,
...
func (db *DB) GetNPCByNameTx(tx *sql.Tx, storyID, name string) (*NPC, error) {
	npc := &NPC{}
	err := tx.QueryRow(
		`SELECT id, story_id, canonical_entity_id, name, role, appearance, personality_json, private_thoughts,
...
func (db *DB) updateNPCWithExec(exec interface {
	...
	_, err := exec.Exec(
		`UPDATE npcs SET name = ?, role = ?, appearance = ?,
         personality_json = ?, private_thoughts = ?, relationship_json = ?, nemesis_json = ?, discovery_json = ?,
         notes_on_protagonist = ?, desires = ?, disposition = ?, is_alive = ?, can_help = ?, last_seen_turn = ?,
         updated_at = ? WHERE id = ?`,
		npc.Name, npc.Role, npc.Appearance,
		npc.PersonalityJSON, npc.PrivateThoughts, npc.RelationshipJSON, npc.NemesisJSON, npc.DiscoveryJSON, npc.NotesOnProtagonist,
