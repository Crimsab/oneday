...
func createChapterExec(exec sqlExecer, ch *Chapter) error {
	result, err := exec.Exec(
		`INSERT INTO chapters (story_id, chapter_number, title, summary, start_turn, end_turn, created_at, branch_id, source_commit_id)
		 VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
		ch.StoryID, ch.ChapterNumber, ch.Title, ch.Summary, ch.StartTurn, ch.EndTurn, ch.CreatedAt, ch.BranchID, ch.SourceCommitID,
	)
	if err != nil {
...
func (db *DB) GetChapter(storyID string, chapterNumber int) (*Chapter, error) {
	...
	err := db.conn.QueryRow(
		`SELECT id, story_id, chapter_number, title, summary, start_turn, end_turn, created_at, branch_id, source_commit_id
         FROM chapters WHERE story_id = ? AND chapter_number = ?`,
		storyID, chapterNumber,
	).Scan(&ch.ID, &ch.StoryID, &ch.ChapterNumber, &ch.Title, &ch.Summary, &ch.StartTurn, &ch.EndTurn, &ch.CreatedAt, &ch.BranchID, &ch.SourceCommitID)
	if err != nil {
...
func (db *DB) ListChapters(storyID string) ([]Chapter, error) {
	...
	for rows.Next() {
		var ch Chapter
		if err := rows.Scan(&ch.ID, &ch.StoryID, &ch.ChapterNumber, &ch.Title, &ch.Summary, &ch.StartTurn, &ch.EndTurn, &ch.CreatedAt, &ch.BranchID, &ch.SourceCommitID); err != nil {
