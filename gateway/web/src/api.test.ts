...
describe("api request handling", () => {
  ...
  it("returns JSON payloads from the gateway", async () => {
    mockFetch(
      new Response(JSON.stringify([{ id: "story", name: "Story" }]), {
        status: 200,
      }),
    );
    await expect(getStories()).resolves.toMatchObject([
      { id: "story", name: "Story" },
    ]);
  });
  ...
  it("checks the browser session before protected application requests", async () => {
    mockFetch(
      new Response(
        JSON.stringify({ authenticated: false, bootstrap_available: true }),
        { status: 200 },
      ),
    );
  ...
  it("reads the protected installation readiness report", async () => {
    mockFetch(
      new Response(
        JSON.stringify({
          probes: [{ name: "narrative", status: "ready", required: true }],
        }),
        { status: 200 },
      ),
    );

    await expect(getSetupReadiness()).resolves.toMatchObject({
      probes: [{ name: "narrative", required: true }],
    });
    expect(globalThis.fetch).toHaveBeenCalledWith(
      "/api/setup/readiness",
      expect.objectContaining({ signal: expect.any(AbortSignal) }),
  ...
  it("reads server-side model discovery without sending endpoint details", async () => {
    ...
    await expect(getModelDiscovery()).resolves.toMatchObject({
      sources: [{ id: "litellm", models: ["story-model"] }],
    });
    expect(globalThis.fetch).toHaveBeenCalledWith(
      "/api/config/model-discovery",
      expect.objectContaining({ signal: expect.any(AbortSignal) }),
    );
  });
  ...
  it("requests a fresh server-side discovery only when explicitly refreshed", async () => {
    ...
    expect(globalThis.fetch).toHaveBeenCalledWith(
      "/api/config/model-discovery?refresh=true",
      expect.objectContaining({ signal: expect.any(AbortSignal) }),
    );
  });
  ...
  it("requests localized command presentation while preserving stable command tokens", async () => {
          ...
          JSON.stringify([
            {
              ...
              canonical: "save",
              aliases: ["s"],
              title: locale === "it" ? "Salva" : "Save",
              description:
                locale === "it" ? "Crea un salvataggio." : "Create a save.",
            },
          ]),
          { status: 200 },
        ),
    ...
    expect([english[0].description, italian[0].description]).toEqual([
      "Create a save.",
      "Crea un salvataggio.",
    ]);
    expect({
      canonical: italian[0].canonical,
      aliases: italian[0].aliases,
    }).toEqual({
      canonical: english[0].canonical,
      aliases: english[0].aliases,
    });
  });
  ...
  it("normalizes omitted empty audio collections", async () => {
    ...
    await expect(getMessageAudio("story-1", 42)).resolves.toEqual({
      assets: [],
      jobs: [],
    });
  });
  ...
  it("cancels visual generation jobs through the story gateway", async () => {
    ...
    await expect(
      cancelVisualGenerationJob("story-1", 12),
    ).resolves.toMatchObject({
      jobs: [{ id: 12, status: "cancelled" }],
    });
  ...
  it("submits exact visual operations with fallback forbidden", async () => {
    mockFetch(
      new Response(JSON.stringify({ profile: {}, assets: [], jobs: [] }), {
        status: 200,
      }),
    );
  ...
  it("requests visual asset cleanup without exposing file contents", async () => {
    ...
    await expect(
      cleanupVisualAssetFiles("story-1", { dry_run: true }),
    ).resolves.toMatchObject({
      story_id: "story-1",
      dry_run: true,
      deleted_files: ["/tmp/stale.png"],
  ...
  it("patches story metadata and archive state", async () => {
    mockFetch(
      new Response(
        JSON.stringify({ id: "story-1", name: "Renamed", is_archived: true }),
        { status: 200 },
      ),
    );

    await expect(
      updateStory("story-1", { name: "Renamed", is_archived: true }),
    ).resolves.toMatchObject({
      id: "story-1",
      name: "Renamed",
      is_archived: true,
  ...
  it("deletes stories through the gateway", async () => {
    mockFetch(
      new Response(JSON.stringify({ story_id: "story-1" }), { status: 200 }),
    );

    await expect(deleteStory("story-1")).resolves.toEqual({
      story_id: "story-1",
    });
    expect(globalThis.fetch).toHaveBeenCalledWith(
      "/api/stories/story-1",
  ...
  it("loads story delete plans before destructive deletion", async () => {
    ...
    await expect(getStoryDeletePlan("story-1")).resolves.toMatchObject({
      story_id: "story-1",
      total_rows: 42,
    });
    expect(globalThis.fetch).toHaveBeenCalledWith(
      "/api/stories/story-1/delete-plan",
      expect.objectContaining({ signal: expect.any(AbortSignal) }),
    );
  });
  ...
  it("uses the branch timeline contract for reads and guarded mutations", async () => {
    ...
    await expect(getTimeline("story/one")).resolves.toMatchObject({
      active_branch_id: "branch-main",
      revision: 7,
    });
    expect(globalThis.fetch).toHaveBeenCalledWith(
    ...
    mockFetch(
      new Response(
        JSON.stringify({
          timeline: { active_branch_id: "branch-alt" },
          snapshot: { story: { id: "story/one" } },
        }),
        { status: 200 },
      ),
    );
    await updateTimeline("story/one", {
      action: "checkout",
      client_revision: 7,
    ...
    expect(globalThis.fetch).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        method: "POST",
        body: expect.stringContaining('"client_revision":7'),
      }),
    );
    ...
    mockFetch(
      new Response(
        JSON.stringify({
          timeline: { active_branch_id: "branch-fork" },
          snapshot: { story: { id: "story/one" } },
        }),
        { status: 200 },
      ),
  ...
  it("recovers timeline reads from transient server failures", async () => {
    mockFetchSequence([
      new Response(JSON.stringify({ error: "temporary timeline failure" }), {
        status: 500,
      }),
      new Response(
        JSON.stringify({
    ...
    await expect(getTimeline("story-1")).resolves.toMatchObject({
      revision: 9,
    });
  ...
  it("requests branch-scoped paginated history, chapters, and exports", async () => {
    ...
    expect(globalThis.fetch).toHaveBeenCalledWith(
      "/api/stories/story-1/history?limit=40&q=glass+seal&scope=current&cursor=41",
      expect.objectContaining({ signal: expect.any(AbortSignal) }),
    );
    ...
    expect(globalThis.fetch).toHaveBeenCalledWith(
      "/api/stories/story-1/history?limit=40&q=&scope=branch&branch_id=branch-alt",
      expect.objectContaining({ signal: expect.any(AbortSignal) }),
    );
    ...
    expect(globalThis.fetch).toHaveBeenCalledWith(
      "/api/stories/story-1/chapters?limit=30&q=arrival&scope=current&cursor=3",
      expect.objectContaining({ signal: expect.any(AbortSignal) }),
    );
    ...
    await expect(getStoryExport("story-1", "json")).resolves.toMatchObject({
      filename: "history.json",
    });
    expect(globalThis.fetch).toHaveBeenCalledWith(
      "/api/stories/story-1/export?format=json",
      expect.objectContaining({ signal: expect.any(AbortSignal) }),
    );
  });
  ...
  it("loads message diagnostics and redacted telemetry exports", async () => {
    mockFetch(
      new Response(JSON.stringify({ run_id: "run-1", attempts: [] }), {
        status: 200,
      }),
    );
    await expect(getMessageDiagnostics("story/one", 42)).resolves.toMatchObject(
      { run_id: "run-1" },
    );
    expect(globalThis.fetch).toHaveBeenCalledWith(
      "/api/stories/story%2Fone/messages/42/diagnostics",
      expect.objectContaining({ signal: expect.any(AbortSignal) }),
    );
    ...
    await expect(getTelemetryExport("story/one", 250)).resolves.toMatchObject({
      format: "jsonl",
      count: 0,
    });
    expect(globalThis.fetch).toHaveBeenCalledWith(
      "/api/stories/story%2Fone/telemetry/export?limit=250",
      expect.objectContaining({ signal: expect.any(AbortSignal) }),
    );
  });
  ...
  it("uses the branch-scoped minigame host endpoints", async () => {
    ...
    mockFetch(
      new Response(JSON.stringify({ instance: { id: "mini-1" } }), {
        status: 200,
      }),
    );
    ...
    expect(globalThis.fetch).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        method: "POST",
        body: expect.stringContaining('"kind":"pattern"'),
      }),
    );
    ...
    mockFetch(
      new Response(
        JSON.stringify({
          instance: { id: "mini-1", runtime: { phase: "resolved" } },
        }),
        { status: 200 },
      ),
    );
    await inputMiniGame("story/one", "mini/1", {
      action: "submit",
      value: "8",
    });
    expect(globalThis.fetch).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        method: "POST",
        body: expect.stringContaining('"value":"8"'),
      }),
    );
  });
  ...
  it("localizes stable API error codes without exposing server prose", async () => {
    ...
    mockFetch(
      new Response(
        JSON.stringify({ code: "stale_config", error: "raw server detail" }),
        { status: 409 },
      ),
    );
    await expect(getStories()).rejects.toMatchObject({
      code: "stale_config",
      message: "La configurazione è cambiata. Aggiorna e riprova.",
    });
  });
  ...
  it("maps public audio and challenge validation codes", async () => {
    await setInterfaceLocale("it");
    mockFetch(
      new Response(JSON.stringify({ code: "invalid_audio_request" }), {
        status: 400,
      }),
    );
    await expect(getStories()).rejects.toMatchObject({
      message: "Controlla le impostazioni dell’audio parlato e riprova.",
    });
    mockFetch(
      new Response(JSON.stringify({ code: "invalid_minigame_request" }), {
        status: 400,
      }),
    );
    await expect(getStories()).rejects.toMatchObject({
      message:
        "L’azione della sfida non è valida. Controlla la scelta e riprova.",
    });
  });
  ...
  it("uses a safe localized fallback for unknown client errors", async () => {
    ...
    await expect(getStories()).rejects.toMatchObject({
      code: "future_error",
      message:
        "Impossibile completare la richiesta. Controlla i dati e riprova.",
    });
  });
});
