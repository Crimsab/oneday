...
async function fetchWithTimeout(
  ...
  try {
    ...
    if (controller.signal.aborted && !callerSignal?.aborted) {
      throw new ApiRequestError(i18n.t("common:requestTimedOut"), 408, {
...
function localizedError(payload: ErrorPayload, status: number): string {
  ...
  if (payload.code && knownCodes.has(payload.code))
    return i18n.t(`api_errors:${payload.code}`);
...
export function bootstrapBrowserSession(
  token: string,
...
export function getAgencyEvents(
  storyId: string,
  limit = 20,
): Promise<AgencyEventView[]> {
  return request<AgencyEventView[]>(
    `/api/stories/${encodeURIComponent(storyId)}/agency-events?limit=${limit}`,
  );
}
...
export function getTTSCatalog(language = ""): Promise<TTSCatalogResponse> {
  ...
  return request<TTSCatalogResponse>(
    `/api/tts/voices${query.size ? `?${query}` : ""}`,
  );
}

export function getTTSSettings(storyId: string): Promise<TTSSettingsResponse> {
  return request<TTSSettingsResponse>(
    `/api/stories/${encodeURIComponent(storyId)}/tts/settings`,
  );
}

export function updateTTSSettings(
  storyId: string,
...
export function getVoiceAssignments(
  storyId: string,
): Promise<VoiceAssignmentsResponse> {
  return request<VoiceAssignmentsResponse>(
    `/api/stories/${encodeURIComponent(storyId)}/voice-assignments`,
  );
}

export function updateVoiceAssignment(
  storyId: string,
  assignment: VoiceAssignment,
  clientRevision: number,
): Promise<VoiceAssignmentsResponse> {
  return request<VoiceAssignmentsResponse>(
    `/api/stories/${encodeURIComponent(storyId)}/voice-assignments/${encodeURIComponent(assignment.id)}`,
    {
      method: "PUT",
      headers: { "content-type": "application/json" },
...
export function getMessageAudio(
  storyId: string,
  messageId: number,
): Promise<MessageAudioResponse> {
  return request<Partial<MessageAudioResponse>>(
    `/api/stories/${encodeURIComponent(storyId)}/messages/${messageId}/audio`,
  ).then(normalizeMessageAudioResponse);
}

export function createMessageAudio(
  storyId: string,
  messageId: number,
): Promise<MessageAudioResponse> {
  return request<Partial<MessageAudioResponse>>(
...
export function retryAudioJob(
  storyId: string,
  jobId: string,
): Promise<MessageAudioResponse> {
  return request<Partial<MessageAudioResponse>>(
    `/api/stories/${encodeURIComponent(storyId)}/audio/jobs/${encodeURIComponent(jobId)}/retry`,
    { method: "POST" },
  ).then(normalizeMessageAudioResponse);
}
...
export function cancelAudioJob(
  ...
  return request<Partial<MessageAudioResponse>>(
    `/api/stories/${encodeURIComponent(storyId)}/audio/jobs/${encodeURIComponent(jobId)}/cancel`,
    { method: "POST" },
  ).then(normalizeMessageAudioResponse);
}

export function normalizeMessageAudioResponse(
  response: Partial<MessageAudioResponse>,
): MessageAudioResponse {
  return {
    ...response,
...
export function getPronunciations(
  storyId: string,
  language = "",
): Promise<PronunciationsResponse> {
  const query = new URLSearchParams();
  if (language) query.set("language", language);
  return request<PronunciationsResponse>(
    `/api/stories/${encodeURIComponent(storyId)}/pronunciations${query.size ? `?${query}` : ""}`,
  );
}
...
export function updatePronunciation(
  ...
  return request<PronunciationsResponse>(
    `/api/stories/${encodeURIComponent(storyId)}/pronunciations/${encodeURIComponent(entry.id)}`,
    {
      method: "PUT",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ ...entry, client_revision: clientRevision }),
    },
  );
}

export function deletePronunciation(
  storyId: string,
  id: string,
  clientRevision: number,
): Promise<PronunciationsResponse> {
  return request<PronunciationsResponse>(
    `/api/stories/${encodeURIComponent(storyId)}/pronunciations/${encodeURIComponent(id)}?client_revision=${clientRevision}`,
    { method: "DELETE" },
  );
}
...
export function cleanupAudio(
  storyId: string,
  dryRun = true,
): Promise<AudioCleanupResponse> {
  return request<AudioCleanupResponse>(
    `/api/stories/${encodeURIComponent(storyId)}/audio/cleanup`,
    {
      method: "POST",
      headers: { "content-type": "application/json" },
...
export function getAudioExport(storyId: string): Promise<AudioExportResponse> {
  return request<AudioExportResponse>(
    `/api/stories/${encodeURIComponent(storyId)}/audio/export`,
  );
}
...
export function deleteStory(storyId: string): Promise<StoryDeleteResponse> {
  return request<StoryDeleteResponse>(
...
export function getStoryDeletePlan(storyId: string): Promise<StoryDeletePlan> {
  return request<StoryDeletePlan>(
    `/api/stories/${encodeURIComponent(storyId)}/delete-plan`,
  );
}

export function runStoryWizard(
...
export function enhanceStoryText(
...
export function getSnapshot(storyId: string): Promise<StorySnapshot> {
  return request<StorySnapshot>(
    `/api/stories/${encodeURIComponent(storyId)}/snapshot`,
  );
}
...
export function getStoryOverview(
  ...
  return request<StoryOverview>(
    `/api/stories/${encodeURIComponent(storyId)}/overview`,
    { signal },
  );
}

export function getStorySaves(
  storyId: string,
  signal?: AbortSignal,
): Promise<SaveView[]> {
  return request<SaveView[]>(
    `/api/stories/${encodeURIComponent(storyId)}/saves`,
    { signal },
  );
}
...
export function sendCraftMessage(
  ...
  return request<CraftResponseEnvelope>(
    `/api/stories/${encodeURIComponent(storyId)}/craft`,
    {
      method: "POST",
      headers: { "content-type": "application/json" },
  ...
  for (let attempt = 0; attempt < 3; attempt += 1) {
    try {
      ...
      await new Promise((resolve) =>
        globalThis.setTimeout(resolve, 120 * (attempt + 1)),
      );
    }
  }
  throw lastError;
...
export function updateTimeline(
  storyId: string,
  payload: TimelineEnvelope,
): Promise<TimelineMutationResponse> {
  return request<TimelineMutationResponse>(
    `/api/stories/${encodeURIComponent(storyId)}/timeline`,
    {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(payload),
...
export function getHistory(
  ...
  search = "",
  signal?: AbortSignal,
  scope: "current" | "all" | "branch" = "current",
  branchId = "",
): Promise<HistoryPage> {
  const query = new URLSearchParams({ limit: "40", q: search, scope });
  if (cursor) query.set("cursor", String(cursor));
  if (branchId) query.set("branch_id", branchId);
  return request<HistoryPage>(
    `/api/stories/${encodeURIComponent(storyId)}/history?${query}`,
    { signal },
  );
}
export function getChapters(
  ...
  const query = new URLSearchParams({ limit: "30", q: search, scope });
  if (cursor) query.set("cursor", String(cursor));
  if (branchId) query.set("branch_id", branchId);
  return request<ChapterPage>(
    `/api/stories/${encodeURIComponent(storyId)}/chapters?${query}`,
    { signal },
  );
}
export function getStoryExport(
  storyId: string,
  format: "markdown" | "html" | "txt" | "json" | "epub" | "replay",
  language = "",
  mode: "original" | "translated" | "bilingual" = "original",
): Promise<StoryExport> {
  const query = new URLSearchParams({ format });
  if (language) query.set("language", language);
  if (mode !== "original") query.set("mode", mode);
  return request<StoryExport>(
    `/api/stories/${encodeURIComponent(storyId)}/export?${query}`,
  );
}
export async function getStoryEpub(
  storyId: string,
  language = "",
  mode: "original" | "translated" | "bilingual" = "original",
): Promise<{ filename: string; blob: Blob }> {
  const query = new URLSearchParams({ format: "epub" });
  if (language) query.set("language", language);
  if (mode !== "original") query.set("mode", mode);
  ...
  if (!response.ok) {
    ...
    throw new ApiRequestError(
      localizedError(payload, response.status),
      response.status,
      payload,
    );
  }
...
export function getMessageDiagnostics(
  ...
  return request<GenerationDiagnostics>(
    `/api/stories/${encodeURIComponent(storyId)}/messages/${encodeURIComponent(String(messageId))}/diagnostics`,
  );
}
export function getTelemetryExport(
  ...
  return request<TelemetryExport>(
    `/api/stories/${encodeURIComponent(storyId)}/telemetry/export?limit=${encodeURIComponent(String(limit))}`,
  );
}
...
export function getVisualAssets(
  ...
  return request<VisualAssetsResponse>(
    `/api/stories/${encodeURIComponent(storyId)}/visual-assets`,
    { signal },
  );
}

export function getActiveMiniGame(storyId: string): Promise<MiniGameResponse> {
  return request<MiniGameResponse>(
    `/api/stories/${encodeURIComponent(storyId)}/minigames`,
  );
}

export function startMiniGame(
  storyId: string,
  kind: MiniGameKind | null,
...
export function inputMiniGame(
  ...
  return request<MiniGameResponse>(
    `/api/stories/${encodeURIComponent(storyId)}/minigames/${encodeURIComponent(instanceId)}/input`,
    {
      method: "POST",
      headers: { "content-type": "application/json" },
...
export function updateVisualProfile(
  storyId: string,
  payload: VisualProfileUpdate,
): Promise<VisualAssetsResponse> {
  return request<VisualAssetsResponse>(
    `/api/stories/${encodeURIComponent(storyId)}/visual-profile`,
    {
      method: "PUT",
      headers: { "content-type": "application/json" },
      body: JSON.stringify(payload),
...
export function generateVisualAssets(
  storyId: string,
  payload: GenerateVisualAssetsRequest = {},
): Promise<VisualAssetsResponse> {
  return request<VisualAssetsResponse>(
    `/api/stories/${encodeURIComponent(storyId)}/visual-assets/generate`,
    {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify(payload),
...
export function cancelVisualGenerationJob(
  storyId: string,
  jobId: number,
): Promise<VisualAssetsResponse> {
  return request<VisualAssetsResponse>(
...
export function getVisualAssetVersions(
  storyId: string,
  assetId: string,
): Promise<VisualAssetVersion[]> {
...
export function selectVisualAssetVersion(
  storyId: string,
  assetId: string,
  versionId: number,
): Promise<VisualAssetsResponse> {
...
export function stepVisualAssetSelection(
  storyId: string,
  assetId: string,
  action: "undo" | "redo",
): Promise<VisualAssetsResponse> {
...
export function getCommandDescriptors(
  ...
  return request<CommandDescriptor[]>(
    `/api/contracts/commands?locale=${encodeURIComponent(locale)}`,
  );
}
...
export function getModelDiscovery(refresh = false): Promise<ModelDiscovery> {
  return request<ModelDiscovery>(
    `/api/config/model-discovery${refresh ? "?refresh=true" : ""}`,
  );
}
...
export function submitAction(
  ...
  return request<ActionResponse>(
    `/api/stories/${encodeURIComponent(storyId)}/actions`,
    {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify(envelope),
...
export function submitMeta(
  ...
  return request<MetaResponse>(
    `/api/stories/${encodeURIComponent(storyId)}/meta`,
    {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify(envelope),
...
export function deleteSave(
  ...
  return request<DeleteSaveResponse>(
    `/api/stories/${encodeURIComponent(storyId)}/saves/${encodeURIComponent(envelope.save_id)}`,
    {
      method: "DELETE",
      headers: { "content-type": "application/json" },
      body: JSON.stringify(envelope),
